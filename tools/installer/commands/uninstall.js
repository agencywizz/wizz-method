...
module.exports = {
  command: 'uninstall',
  description: 'Remove Wizz installation from the current project',
  options: [
    ['-y, --yes', 'Remove all Wizz components without prompting (preserves user artifacts)'],
    ...
    try {
      ...
      if (options.directory) {
        ...
        const dirChoice = await prompts.select({
          message: 'Where do you want to uninstall Wizz from?',
          choices: [
      ...
      await prompts.intro('Wizz Uninstall');
      ...
      await prompts.outro('To reinstall, run: npx wizz-method install');
